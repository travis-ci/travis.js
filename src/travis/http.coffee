    ...
    promise = new Travis.Promise (promise) ->
      ...
      sendRequest = (opt, updatedOpt) ->
        ...
        http.rawRequest opt, (status, headers, body) ->
          # most of the time the underlying http implementation will take care of redirects
          # but in the spirit of portability, we should not rely on that
          switch status
